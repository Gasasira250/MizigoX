  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-8">
      ...
      <p className="mt-2 text-sm text-slate-600">
        This page is not available for your role. If you think this is a mistake, ask an administrator.
      </p>
      <Link className="mt-4 inline-block text-sm font-medium text-teal-800 hover:underline" to={homePathFor(user)}>
        Back to home
      </Link>
    </section>
  );
...
export function UnauthorizedPage() {
  return (
    <section className="mx-auto mt-16 max-w-md rounded-2xl border border-slate-200 bg-white p-8">
      ...
      <Link className="mt-4 inline-block rounded-md bg-[#12355b] px-4 py-2 text-sm text-white" to="/login">
        Sign in
      </Link>
    </section>
  );
