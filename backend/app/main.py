...
async def lifespan(_app: FastAPI):
    ...
    try:
        ...
        raise RuntimeError(
            f"Database startup failed using DATABASE_URL={DATABASE_URL!r}. "
            "For local demos leave DATABASE_URL unset (SQLite). "
            "For Postgres, ensure the server is running and the URL is correct."
        ) from exc
    yield
...
@app.get("/health")
def health():
    from app.database import database_engine_label

    return {
        "ok": True,
        "service": "mizigox",
        "database": database_engine_label(),
    }
